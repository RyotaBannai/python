...
## 9. クラス
- クラスの継承メカニズムは, 複数の基底クラスを持つことができ, 派生クラスで基底クラスの任意のメソッドをオーバライドすることができる.
- 通常のクラスメンバ (データメンバも含む) は (プライベート変数 に書かれている例外を除いて) public であり, メンバ関数はすべて 仮想関数(virtual) である.
### 9.1. 名前とオブジェクトについて
- 別名付け(alias)は, リストや辞書や他の多くの型など, 変更可能な型を扱う Python コード上で驚くべき効果がある.
### 9.2. Python のスコープと名前空間
- 属性: ドットに続く名前すべてに対して使う. 例えば, 式 z.real で, real はオブジェクト z の属性である.
- 関数のローカルな名前空間は, 関数が呼び出されたときに作成され, 関数から戻ったときや, 関数内で例外が送出され, かつ関数内で処理されなかった場合に削除される. 再帰呼出しのときには, 各々の呼び出しで各自のローカルな名前空間が存在する.
- スコープ (scope) : ある名前空間が直接アクセスできるような, Python プログラムのテキスト上の領域.  "直接アクセス可能" とは, 修飾なしに (訳注: spam.egg ではなく単に egg のように) 名前を参照した際に, その名前空間から名前を見つけようと試みること. 
- スコープの探索方法はcと同じ.最後はビルドイン名を探索.
- nonlocal 文は, 列挙された識別子がグローバルを除く _一つ外側のスコープ_ で先に束縛された変数を参照するようにする. これは, 束縛のデフォルトの動作が _まずローカル名前空間_ を探索するので重要である. この文は, 中にあるコードが, グローバル (モジュール) スコープ以外の _ローカルスコープの外側の変数_ を再束縛できるようにする. 
- 関数の外側では, ローカルスコープはグローバルな名前空間と同じ名前空間, モジュールの名前空間を参照する クラス定義では, ローカルスコープの中にもう一つ名前空間が置かれる.
- Python の特徴として, global や nonlocal 文が有効でない場合は, _名前に対する参照は常に最も内側のスコープに対して有効_. 代入はオブジェクトを名前に束縛するだけでコピーは_しない_. 削除も同様で, del x は, ローカルスコープの名前空間から x に対する拘束を取り除く. つまるところ, 新しい名前を与えるようなすべての操作は, ローカルスコープを使って行われる. _import 文, 関数の定義_ は, モジュールや関数名をローカルスコープの名前に拘束する. `@tutorial/scope.py`
#### 9.3.1 クラス定義の構文
- クラス定義から普通に (定義の終端に到達して) 抜けると, クラスオブジェクト (class object) が生成される. _クラスオブジェクト_ は, 基本的にはクラス定義で作成された名前空間の内容をくるむラッパ (wrapper) である.  (クラス定義に入る前に有効だった) 元のローカルスコープが復帰し, 生成されたクラスオブジェクトは _復帰したローカルスコープ_ に _クラス定義のヘッダで指定した名前_ で結び付けられる.
#### 9.3.2. クラスオブジェクト
- 属性参照 (attribute reference) : Python におけるすべての属性参照で使われている標準的な構文, obj.name を使う.
- \_\_init()\_\_がconstructor
#### 9.3.3. インスタンスオブジェクト
- インスタンス属性 1, データ属性 (data attribute): これは Smalltalk の "インスタンス変数" や C++の "データメンバ" に相当. 
- インスタンス属性 2, メソッド (method): オブジェクトに "属している" 関数. (関数オブジェクトではなく、_メソッドオブジェクト (method object)_ である.)
- メソッドについて特別なこととして, _インスタンスオブジェクトが関数の第1引数として渡される_. 例えば, _x = MyClass; x.f() という呼び出しは, MyClass.f(x) と厳密に等価なものである_.
#### 9.3.4. メソッドオブジェクト
- オブジェクトメソッド（抽象オブジェクト）が呼び出されるとき, インスタンスしたときに作成済みの（パック済みの）インスタンスオブジェクトと関数オブジェクトへのポインタを使って, 関数オブジェクトへオブジェクトインスタンスと引数リストを渡して呼び出す. なので, クラスの中の関数オブジェクトの第一仮引数は, インスタンスオブジェクトを渡せるように, _self_ としている.
#### 9.3.5. クラスとインスタンス変数
- インスタンス変数: それぞれのインスタンスについて固有のデータのためのもの.
- クラス変数: そのクラスのすべてのインスタンスによって共有される属性やメソッドのためのもの. `@tutorial/class_and_instance_vals.py`
- 共有データ(つまり, 上記のクラス変数)はリストや辞書のような mutable オブジェクトが関与すると驚くべき効果を持ち得る. つまり, たった一つのリストがすべての インスタンスオブジェクトによって共有されることになり得るからである. => _リストをインスタンス変数として宣言する_ . (self.listのように.)
### 9.4. いろいろな注意点
- インスタンスとクラスの両方で同じ属性名が使用されている場合, _属性検索_ はインスタンスを優先.
- `x = MyClass; x.val = 'test'` この代入は _インスタンス変数_ への代入. 
- 関数定義は, テキスト上でクラス定義の中に入っている必要は無い. 関数オブジェクトをクラスのローカルな変数の中に代入するのも OK. `def f1(): return True ; Class C: f1=f` のように.
- 個々の値はオブジェクトなので, クラス (型 とも言います) を持っている. それは object.\_\_class\_\_ に保持されている.
### 9.5. 継承
- 構文: `class DerivedClassName(BaseClassName):` 
- 要求された属性がクラスに見つからなかった場合, 基底クラスに検索が進む.
- 基底クラスのメソッドを直接呼び出す簡単な方法: `BaseClassName.methodname(self, arguments)`
- _isinstance()_ : インスタンスの型が調べる. isinstance(obj, int) は obj.__class__ が int や int の派生クラスの場合に限り True.
- _issubclass()_ :クラスの継承関係が調べらる. issubclass(bool, int) は True.
#### 9.5.1. 多重継承 (multiple inheritance)
- 構文: `class DerivedClassName(Base1, Base2, Base3):`
- 多重継承において, 親クラスから継承される属性の検索は, _深さ優先で, 左から右に_, そして継承の階層の中で同じクラスが複数出てくる（訳注: [_ダイアモンド継承(diamond diagram)_](https://en.wikipedia.org/wiki/Multiple_inheritance#The_diamond_problem) ）場合に２度探索をしない, と考えることができる. なので, DerivedClassName にある属性が存在しない場合, まず Base1 から検索され, そして（再帰的に） Base1 の基底クラスから検索され, それでも見つからなかった場合は Base2 から検索される,といった具合になる.
- 協調的な _super()_ の呼び出しのためにメソッドの解決順序は動的に変更される. このアプローチは他の多重継承のある言語で _call-next-method_ として知られている. 
- Method Resolution Order or MRO は, object.\_\_mro\_\_ で参照可.
- base Class はobject.\_\_bases\_\_で参照可. [Python’s super() considered super!](https://rhettinger.wordpress.com/2011/05/26/super-considered-super/)
### 9.6. プライベート変数
- オブジェクトの中からしかアクセス出来ない "プライベート" インスタンス変数は, Python には無い.
- Python では, アンダースコアで始まる名前 (例えば _spam) は, (関数であれメソッドであれデータメンバであれ) 非 public なAPIとして扱う.
- _名前マングリング (name mangling)_ : __spam (先頭に二個以上の下線文字、末尾に一個以下の下線文字) という形式の識別子は, _classname__spam へとテキスト置換される. つまり, base classの関数オブジェクトを意図的に name manglingすることで, 継承したclass内で同じ名前の関数オブジェクトも自動的にそのclassのプライベート変数とすることができる. (それでも, 依然としてアクセス可能であるが, 不慮の事故（意図しない命名の衝突, overwrite）を防ぐことは可能.) `@tutorial/name_mangling.py`
### 9.7. 残りのはしばし
- インスタンスメソッドオブジェクトにも属性がある. m.\_\_self\_\_ はメソッド m() の属しているインスタンスオブジェクトで, m.\_\_func\_\_ はメソッドに対応する関数オブジェクト.
### 9.8. イテレータ (iterator)
- コンテナオブジェクト = iterable object = mutable object.
- iter()は\_\_next()\_\_が定義されている(リスト)イテレータオブジェクトを返す. (つまり, forで取り出せるオブジェクトを返す. ) iter に渡すオブジェクトは,  object は反復プロトコル (\_\_iter\_\_() メソッド) か、シーケンスプロトコル (引数が 0 から開始する \_\_getitem\_\_() メソッド) をサポートする集合オブジェクトでないといけない. 裏では for 文はコンテナオブジェクトに対して iter() 関数を呼んでいる. 
...
### 11.1. 出力のフォーマット
- pprint モジュール　で表示されるwidthを制限したり, jsonフォーマット等のdepthを制限して表示できる.
- PrettyPrinter で構造が複雑なdict, tuple, listをきれいに表示. indentを指定できる.
- locale モジュールを使えば, 国ごとの通過とかを自動で数値に付与することなどできる. `locale.setlocale(locale.LC_ALL, 'English_United States.1252')` localeのリストは, `locale.locale_alias`. en_US, ja_JP のように`言語表記_カンチュリーコード`でも設定可. 
### 11.2. 文字列テンプレート
- Template モジュールを使えば, imgの名前を一つのキーワードをベースにして連番等に統一できたり, 同じデータをhtml, xmlなど異なるフォーマットに変換するためのテンプレートを事前に作成できる. `@tutorial/template.py`
### 11.4. マルチスレッディング
- スレッド処理 (threading) : 順序的な依存関係にない複数のタスクを分割するテクニック（複数の関数を同時に動かすことができる.）
- start()でマルチスレッド開始. join()でマルチスレッドを停止し, バックグラウンドで動いている処理に限って行う. これが終わったら, その次の処理を行う. queueを使って複数のスレッドからのリクエスト（タスクなど取り出し等.）を一箇所にまとめておいた方設計が簡単.
### 11.5. ログ記録
- 最も簡単な使い方では, ログメッセージをファイルや sys.stderr に送信.
### 11.6. 弱参照
- Python は自動的にメモリを管理. (ほとんどのオブジェクトは参照カウント方式で管理し、 ガベージコレクション で循環参照を除去). オブジェクトに対する最後の参照がなくなってしばらくするとメモリは解放される. 
- `weakref モジュール`: オブジェクトへの参照を作らずに追跡するためのツール. 弱参照オブジェクトが不要になると, 弱参照 (weakref) テーブルから自動的に除去され, コールバック関数がトリガされる. 弱参照を使う典型的な応用例には, 作成コストの大きいオブジェクトのキャッシュがある. インスタンスオブジュエクトを作成後, 消去した後に, gc.collect()をすると, そのタイミングでメモリが解放される. 作成コストの大きいオブジェクトは, weakrefに格納しておくといい.
### 11.7. リスト操作のためのツール
- array() オブジェクトは, 配列はリストに似ているが, 同じ形式のデータだけが保存でき, よりコンパクトに保存される. 通常 1 要素あたり 16 バイトを必要とする Python 整数型のリストの 代りに, 2 バイトの符号無しの 2 進数(unsigned short)のみの様相を格納できたりする. `array('H', [4000, 10, 700, 22222])`
